...
- **JM Construct-plan** (`JM-CONSTRUCT-PLAN.md`, concept 0.4; ook als PDF: `JM-Construct-chassisplanning-0.4.pdf`): voorstel voor de aparte chassisplanning. Intern gevalideerd door Damian (24-07). Kernpunten 0.4: **IO-nummer per chassis** (geen order-IO meer); een order is een **batch chassis** die via een **inlaadform** wordt ingevoerd; **PR-nummer per chassis uit Business Central, handmatig** ingevuld; **revisienummer** open tekstveld (Expandable), per order; één trailertype per order. **Fysieke statusflow per chassis**: in productie → chassisbalk klaar → opgeroepen → afmaken → gereed voor ophalen → opgehaald. **Business Central 40/60**: elke IO = twee artikelen; 40% + op voorraad geboekt bij chassisbalk klaar (blijft fysiek bij JM), 60% bij ophalen. Order + notities hebben een bijlagenveld (elk bestandstype). Vervangt het losse Excel-overzicht (§9 = uitgangspunten U1–U12). Nog NIET gebouwd — §10 openstaand: **doorlooptijden/procestijden nog te bepalen**, afroeptermijn, slots, meldniveau. PDF via headless Chrome uit een print-HTML (scratchpad).
