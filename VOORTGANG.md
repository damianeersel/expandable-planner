...
- **JM Construct-plan** (`JM-CONSTRUCT-PLAN.md`, concept 0.3; ook als PDF: `JM-Construct-chassisplanning-0.3.pdf`): voorstel voor de aparte chassisplanning. Intern gevalideerd door Damian (24-07): PR-nummer wordt **handmatig uit Business Central** ingevuld bij orderplaatsing (niet auto-gegenereerd), IO = inkoopnummer, JM-ordernummer vult JM later zelf aan, revisienummer is een open tekstveld dat Expandable zelf invult, gerede chassis blijven op voorraad bij JM tot afroep voor productie, één trailertype per order, order + notities krijgen een bijlagenveld (elk bestandstype), planning vervangt het losse Excel-overzicht (§9 = vastgestelde uitgangspunten U1–U11). Nog NIET gebouwd — eerst §10 (afroeptermijn, slots, werkwijze) afstemmen met JM, daarna Fase A. PDF gegenereerd via headless Chrome uit een print-HTML (scratchpad).
